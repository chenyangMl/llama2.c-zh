...
void error_usage() {
    ...
    fprintf(stderr, "  -k <string> path of tokenzier.bin. default tokenizers/llama2en/tokenizer.bin \n");
    exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 2; i < argc; i+=2) {
        ...
        else if (argv[i][1] == 'k') { tokenizerBin = argv[i + 1]; }
        else { error_usage(); }
    }
    ...
    {
        FILE *file = fopen(tokenizerBin, "rb");
